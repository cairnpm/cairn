  ...
  return {
    ...
    propose: async (input: ProposeInput) => {
      ...
      const text = await callClaude(
        ...
        + 'Do NOT append to a candidate merely to ANNOTATE it — to note that this signal affects, depends on, or renames '
        + 'something in it. A cross-feature dependency is NOT an append: if the signal is itself a shapeable problem (its own '
        + 'problem, solution and scope), CREATE it as its own feature; the dependency is then handled by SEQUENCING at the '
        + 'betting table, never by injecting a note about feature A into feature B\'s pitch. '
        + 'A technical, refactoring or internal-plumbing signal IS a full feature when it carries its own problem, appetite '
        + 'and risks — "it\'s just internal" or "not user-facing" is NEVER a reason to downgrade it to an annotation on another feature. '
        ...
        + 'is too embryonic ("on ne sait pas encore", "à définir"). maturity is decided AFTER you have already '
        + 'chosen to CAPTURE, and never turns a capture into a discard: an unmade strategic decision is a real '
        + 'signal — capture it as "shaping" with its open questions, never discard it as vague or premature. '
        + 'A "shaping" feature is CAPTURED, not discarded '
        ...
        + 'Keep every proposed_spec field TERSE and PLAIN: a few short sentences of running prose. NO markdown — no headings, '
        + 'no **bold**, no numbered/bulleted lists, no backtick-decorated code fences. The field label already gives the '
