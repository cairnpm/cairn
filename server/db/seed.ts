...
export function seedIfEmpty(): void {
  ...
  tx(() => {
    ...
    for (const f of FEATURES) {
      run(
        `INSERT INTO features (id, title, problem, appetite, out_of_bounds, status, stale, hill_id, signal_count, embedding, created_at, updated_at)
         VALUES (?, ?, ?, ?, NULL, ?, 0, ?, ?, ?, ?, ?)`,
        f.id, f.title, f.problem, f.appetite, f.status, f.hill ?? null, f.signal, emb(`${f.title} ${f.problem}`), now, now,
      )
      ...
      f.feedback.forEach((fb, i) => {
        run(
          `INSERT INTO feedback (id, content, source, captured_by, classification, status, feature_id, content_hash, embedding, created_at)
           VALUES (?, ?, ?, 'seed', ?, 'routed', ?, ?, ?, ?)`,
          `fb-${f.id}-${i}`, fb.content, fb.source, fb.classification, f.id, `seed-${f.id}-${i}`, emb(fb.content), now,
        )
      })
