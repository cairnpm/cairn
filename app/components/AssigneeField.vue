  ...
  <div>
    ...
    <div class="flex flex-wrap items-center gap-1.5">
      <span v-for="a in assignees" :key="a.user_id" class="inline-flex items-center gap-1.5 rounded-full border bg-muted/40 py-0.5 pl-0.5 pr-2 text-sm">
        ...
        {{ a.name }}
        <button type="button" class="text-muted-foreground hover:text-foreground" :title="`Retirer ${a.name}`" @click="emit('remove', a.user_id)"><X class="size-3" /></button>
      </span>
      <DropdownMenu>
        <DropdownMenuTrigger as-child>
          <Button variant="outline" size="sm" class="h-7 gap-1 rounded-full border-dashed text-muted-foreground"><Plus class="size-3.5" /> Ajouter</Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="start">
          ...
          <div v-if="!available.length" class="px-2 py-1.5 text-xs text-muted-foreground">Tous les membres sont assignés.</div>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
  </div>
