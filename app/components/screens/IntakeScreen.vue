...
const committed = ref<{ action: string; feature_id: string } | null>(null)
...
const chips = ['Export PDF lent', 'Intégration Slack', 'Bug critique prod', 'Feature onboarding']
  ...
  <div style="height: 100%; display: flex; flex-direction: column; background: white;">
      ...
      <div style="width: 100%; max-width: 600px; display: flex; flex-direction: column; align-items: center; gap: 32px;">
        <div style="display: flex; flex-direction: column; align-items: center; gap: 16px;">
          ...
          <div style="text-align: center;">
            ...
            <div style="font-size: 15px; color: #71717a; line-height: 1.65; max-width: 460px;">Décrivez une feature, un bug ou une urgence.<br>Je détecte les doublons et route vers la bonne feature — l'écriture n'a lieu qu'à la confirmation.</div>
          </div>
        </div>
    ...
    <template v-else>
      <div ref="chatEl" style="flex: 1; min-height: 0; overflow-y: auto; padding: 32px; display: flex; flex-direction: column; gap: 20px;">
        ...
        <div v-if="proposal" style="flex-shrink: 0; max-width: 720px; width: 100%; margin: 0 auto; border: 1.5px solid #18181b; border-radius: 12px; overflow: hidden;">
          <div style="padding: 12px 16px; background: #18181b; color: white; display: flex; align-items: center; gap: 8px; font-size: 13px; font-weight: 600;">
            <span style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; opacity: 0.7;">Décision de routing</span>
          ...
          <div style="padding: 16px;">
            ...
            <div style="display: flex; gap: 8px; margin-top: 14px;">
              <button class="bk-dark" :disabled="pending" style="flex: 1; padding: 9px; background: #18181b; color: white; border: none; border-radius: 7px; font-size: 14px; font-weight: 500; cursor: pointer; font-family: inherit;" @click="accept">✓ Confirmer & écrire</button>
        ...
        <div v-if="committed" style="flex-shrink: 0; max-width: 720px; width: 100%; margin: 0 auto; border: 1px solid #bbf7d0; background: #f0fdf4; border-radius: 10px; padding: 14px 16px; display: flex; align-items: center; gap: 12px;">
          ...
          <div style="flex: 1; font-size: 13px; color: #15803d;">
            Écrit en base — {{ committed.action === 'append' ? 'rattaché à' : 'nouvelle feature' }} <code>{{ committed.feature_id.slice(0, 8) }}</code>.
          </div>
